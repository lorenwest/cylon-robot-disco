...
proto.connect = function(callback) {
  ...
  my.robot.events = my.robot.events || [];
  my.robot.events.push('disco_robot_arrived', 'disco_robot_left');
    ...
    my.disco.on('added', function(newNode) {
      ...
      if (robots) {
        for (var robotName in robots) {
          ...
          my.robot.emit('disco_robot_arrived', robots[robotName]);
        }
        my.connectTheBots();
      }
    });
    my.disco.on('removed', function(oldNode) {
      ...
      if (robots) {
        for (var robotName in robots) {
          ...
          my.robot.emit('disco_robot_left', robots[robotName]);
        }
        my.connectTheBots();
      }
    });
...
proto.advertise = function() {
  ...
  if (!api && Cylon.apiInstances && Cylon.apiInstances.length) {
    api = Cylon.apiInstances[0];
  }
  if (!api) {
    return;
  }
